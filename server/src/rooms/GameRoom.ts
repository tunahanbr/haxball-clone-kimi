...
import { PhysicsEngine, PhysicsConfig, PlayerInput } from "../physics/PhysicsEngine";
...
export class GameRoom extends Room<GameState> {
  ...
  private returnToLobbyTimer: ReturnType<typeof setTimeout> | null = null;
  ...
  onCreate() {
    ...
    this.onMessage("set_team", (client, team: "red" | "blue") => {
      const lobbyPlayer = this.state.lobbyPlayers.get(client.sessionId);
      if (!lobbyPlayer || this.state.roomStatus !== "lobby") return;
      lobbyPlayer.team = team;
      lobbyPlayer.isReady = false;
    });

    this.onMessage("set_ready", (client, isReady: boolean) => {
      const lobbyPlayer = this.state.lobbyPlayers.get(client.sessionId);
      if (!lobbyPlayer || this.state.roomStatus !== "lobby") return;
      lobbyPlayer.isReady = isReady;
    });

    this.onMessage("set_match_settings", (client, settings: { scoreLimit?: number; timeLimitMs?: number; powerUpsEnabled?: boolean }) => {
      if (client.sessionId !== this.state.hostSessionId) return;
      if (this.state.roomStatus !== "lobby") return;
      if (typeof settings.scoreLimit === "number") this.state.scoreLimit = Math.max(0, Math.min(10, settings.scoreLimit));
      if (typeof settings.timeLimitMs === "number") this.state.timeLimitMs = Math.max(0, settings.timeLimitMs);
      if (typeof settings.powerUpsEnabled === "boolean") this.state.powerUpsEnabled = settings.powerUpsEnabled;
    });
    ...
    this.onMessage("start_match", (client) => {
      ...
      const nonHostPlayers: LobbyPlayer[] = [];
      this.state.lobbyPlayers.forEach((p) => {
        if (!p.isHost) nonHostPlayers.push(p);
      });
      ...
      this.state.lobbyPlayers.forEach((lobbyPlayer, sessionId) => {
        this.physics.addPlayer(sessionId, this.state, lobbyPlayer.team as "red" | "blue");
      });
    ...
    this.onMessage("input", (client, message: PlayerInput) => {
      if (this.state.roomStatus !== "playing") return;
      this.physics.setInput(client.sessionId, message);
    });

    this.onMessage("physics_config", (_client, message: Partial<PhysicsConfig>) => {
      this.physics.setConfig(message);
    });
  }

  private gameTick(deltaMs: number) {
    if (this.state.roomStatus === "lobby" || this.state.roomStatus === "game_over") return;
  ...
  onDispose() {
    if (this.returnToLobbyTimer) clearTimeout(this.returnToLobbyTimer);
  }
}
